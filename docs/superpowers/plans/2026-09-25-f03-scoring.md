...
- Commits gitmoji, message au présent en français, emoji Unicode, avec en pied :
  ...
  ```
- Commande de test ciblée : `./node_modules/.bin/vitest run <fichier>` (pas `pnpm vitest`, réécrit par le hook RTK : voir QUIRKS) ; vérification complète : `pnpm check` (format, lint, types, tests).
