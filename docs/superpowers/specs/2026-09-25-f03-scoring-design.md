...
Références : [`PRODUCT.md`](https://github.com/AdrienGras/questionator-z4000-hyperdrive/blob/main/PRODUCT.md) §5, §7 et F03 · [`docs/DECISIONS.md`](https://github.com/AdrienGras/questionator-z4000-hyperdrive/blob/main/docs/DECISIONS.md) D01–D03, D19–D21, D31, D42, D43.
...
src/scoring/
  milli.ts                 Milli, toMilli, fromMilli, assertSafe, hasAtMostThreeDecimals
...
`src/config/rules.ts` abandonne ses aides locales `toThousandths` et le test des 3 décimales au profit de `toMilli` et `hasAtMostThreeDecimals` importés de `src/scoring/milli.ts` : une seule conversion vers les millièmes dans tout le code.
...
- `assertSafe(n)` : lève `RangeError` si `!Number.isSafeInteger(n)`.
- `toMilli(n)` = `assertSafe(Math.round(n × 1000))` ; seule conversion décimal → entier.
...
- `fraction(num, den)` applique `assertSafe` aux deux termes et refuse `den ≤ 0`. Pas de simplification par PGCD (inutile aux ordres de grandeur en jeu).
...
- Résultat `q' × step`, passé à `assertSafe`.
...
Arrondir puis borner (D20) : 0 et `finalScale` toujours atteignables, jamais dépassés. Ordre de grandeur au pire : `capped × scale` ≈ 10⁶ × 10⁵ = 10¹¹, `den × step` ≈ 10⁹, loin de 2⁵³ (D21) ; la garde lève une erreur au-delà.
...
| `isValidAdjustment(value, config)` | `Number.isFinite(value)`, `hasAtMostThreeDecimals(value)` et `toMilli(value) % step === 0`. Pas de bornes : le bornage de l'étape 4 s'en charge. |
...
- **rules.ts** : les tests F02 existants passent inchangés après le passage à `milli.ts`.
